...
## Serial link (succorfish_driver)

The serial-facing nodes (`owtt_beacon_node`, `owtt_surface_unit_node`) **no
longer open a serial port** themselves. The sibling submodule package
[`succorfish_driver`](../../../succorfish_driver/README.md) exclusively owns the
Teensy/modem port and must be running. The nodes talk to it over ROS through the
`DriverClient` helper, using the driver's **relative** topic/service names (from
`succorfish_msgs/Topics`):

- `succorfish/tx` (`std_msgs/String`) — outbound command, client→driver (the
  driver appends the terminator)
...
- `succorfish/send_command` (`succorfish_msgs/SendCommand` service) — synchronous
  request/response

Because the names are relative, a node and its driver line up by **namespace**.
The beacon stack uses the Teensy front-end (115200-baud `/dev/ttyACM*` profile),
selected by which `succorfish_driver` config you launch (vs. the 9600-baud
`/dev/ttyUSB*` Succorfish profile). The serial port/baud are configured on the
driver, not here, so there are no `serial_port`/`serial_baudrate` args on these
nodes anymore. The MQTT/inference nodes (`owtt_inference_node`,
`owtt_fake_mqtt`) never touched serial and are unaffected.
...
ros2 launch serial_ping_pkg owtt_beacon_node.launch \
    beacon_name:=lolo own_modem_id:=007 \
...
ros2 launch serial_ping_pkg owtt_surface_unit_node.launch \
    unit_name:=floatsam_usv_0 own_modem_id:=067 \
...
ros2 launch serial_ping_pkg owtt_surface_unit_node.launch \
    unit_name:=floatsam_usv_1 own_modem_id:=069 \
...
### In simulation (smarcUnity)

Run the same nodes with the driver's `unity` backend instead of a serial port.
The bridge translates the modem's transmit/receive against Unity's acoustic
`Transceiver` (`StringStamped` + time-of-flight), so the OWTT range falls out of
the sim geometry. Put each node + its driver in the vehicle's namespace
(`robot_name:=`); Unity publishes the acoustic topics under the same namespace.

```bash
# Beacon on 'sam' — transmit depth (svs/latlon are silent while submerged)
ros2 launch serial_ping_pkg owtt_beacon_node.launch \
...
# Surface unit on 'lolo' tracking sam — sim ToF is geometric, so offset 0 and
# no auto-calibrate; MQTT on so the inference node can read the reports.
ros2 launch serial_ping_pkg owtt_surface_unit_node.launch \
    robot_name:=lolo start_driver:=true driver_backend:=unity \
    own_modem_id:=067 unity_own_modem_id:=067 \
    beacon_name:=sam beacon_modem_id:=101 \
    owtt_offset_us:=0.0 auto_calibrate:=false unit_name:=lolo
```

Notes:
- The beacon only transmits when it has telemetry, so point it at a source the
  sim actually publishes (e.g. `/<robot>/smarc/depth`).
- The inference node ingests range reports over MQTT only, so a single receiver
  needs a seed (`inference.initial_lat/lon`, a seeded beacon GPS, or a second
  receiver for a multilateration fix) before it can start its range-only filter.
- A ready-made tmux bring-up (bridge + beacon + surface unit + topic echoes +
  foxglove + inference) lives at `smarc2/simulation/local/owtt_sim_bringup.sh`.
