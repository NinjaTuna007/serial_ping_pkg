...
class ModemPingEstimatorNode(WireSafeSerialNode):
    ...
    def __init__(self):
        ...
        # ---------------- serial (via succorfish_driver) ----------------
        # Inbound lines are ignored until the node is fully initialised below.
        # Wait for the driver so the one-shot startup wire command below is not
        # published before pub/sub discovery settles (otherwise it is dropped).
        self._ready = False
    ...
    def _tick(self):
        # Inbound lines now arrive via _on_serial_line (driver RX), not polled here.
        if self.state != PingState.RUNNING:
            return True
    ...
    def _publish_estimated_xyz(self, modem_id: str, xyz, sigma: float):
        ...
        self.get_logger().info(
            ...
            f'lat={out.position.latitude:.{LATLON_DECIMALS}f}, '
            f'lon={out.position.longitude:.{LATLON_DECIMALS}f}, '
...
def main(args=None):
    # Disable rclpy's built-in signal handling so the context stays alive long
    # enough for to_wire_mode() to publish + flush the wire-mode command on exit.
    ...
    try:
        spin_wire_safe(node, executor=executor)
    except (KeyboardInterrupt, ExternalShutdownException):
        pass
    finally:
        # Ask the driver to return the Teensy to WIRE mode while ROS is still up.
        node.to_wire_mode()
        executor.shutdown()
        node.destroy_node()
        if rclpy.ok():
