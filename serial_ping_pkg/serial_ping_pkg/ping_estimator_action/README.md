...
> used for the ranging request/reply). Because the names are relative, the node
> lines up with its driver when they share a namespace, and the driver picks the
> serial profile (9600-baud `/dev/ttyUSB*` Succorfish or 115200-baud
> `/dev/ttyACM*` Teensy) via the config you launch. Needs `succorfish_driver` /
> `succorfish_msgs` (not `pyserial`). See
> [`succorfish_driver`](../../../succorfish_driver/README.md).

> **Wire-safe shutdown:** on exit the node returns the Teensy to **wire** mode by
> publishing `$Y<id>W` on `succorfish/tx`. The wire-safe base disables rclpy's
> default signal handlers and runs a manual spin loop so this command is flushed
> to the driver before shutdown (an `atexit` hook is the last resort); only
> `SIGKILL` / power loss or a downed driver can bypass it.
...
Modem line protocol handled by the node (commands go out on `succorfish/tx` /
the `succorfish/send_command` service, replies arrive on `succorfish/rx`):
...
The node talks to the `succorfish_driver` over ROS, so make sure a driver is
running in the same namespace (e.g. `-r __ns:=/lolo` here lines up with a driver
launched with `namespace:=lolo`).
...
|-------|-----------|---------|-------|
| teensy | `teensy.enable_wire_on_startup` | `true` | wire-mode `$Y<id>W` on startup, sent via the driver (harmless on a raw modem) |
| teensy | `teensy.own_modem_id` | `007` | own modem address |
| teensy | `teensy.command_terminator` | `\r\n` | terminator the driver appends to commands |
